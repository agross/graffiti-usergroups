...
namespace DnugLeipzig.Runtime.Tests.Plugins.Events
{
		...
		: Spec
	{
		...
		EventPluginSettings _settings;
		IValidator<EventPluginSettings> _sut;

		protected override void Establish_context()
		{
			var categoryRepository = MockRepository.GenerateMock<ICategoryRepository>();
			_sut = new EventPluginSettingsValidator(categoryRepository);

			categoryRepository.Stub(x => x.Exists(null))
				.IgnoreArguments()
				.Return(false);

			_settings = new EventPluginSettings
			            {
		...
		: With_existing_category
	{
		...
		[RowTest]
		[Row("invalid")]
		public void It_should_return_an_error_for_invalid_values(string email)
		{
			Because(email);
			Assert.AreEqual(Severity.Error, _report.First().Severity);
		}
		...
		[Row("   ")]
		[Row("foo@example.com")]
		public void It_return_successfully_for_valid_values(string email)
		{
			Because(email);
			Assert.AreEqual(0, _report.Count);
		}
		...
		: With_existing_category
	{
		...
		[Row("invalid", "Please enter a valid integer value for the default maximum number of registrations.")]
		...
		[Row("-100")]
		[Row("invalid")]
		public void It_should_return_an_error_for_invalid_values(string numberOfRegistrations)
		{
			Because(numberOfRegistrations);
			Assert.AreEqual(Severity.Error, _report.First().Severity);
		}
		...
		[Row("0")]
		[Row("100")]
		public void It_should_return_successfully_for_valid_values(string numberOfRegistrations)
		{
			Because(numberOfRegistrations);
			Assert.AreEqual(0, _report.Count);
		}
	...
	public abstract class With_existing_category : Spec
	{
		...
		protected IValidator<EventPluginSettings> _sut;

		protected EventPluginSettings Settings
		{
			get;
			private set;
		}
		...
		protected override void Because()
		{
			Settings = new EventPluginSettings
			           {
			           	CategoryName = "talk category",
			           	YearQueryString = "year query string",
